...
for datum in data:
	...
	temp_datum = dict()
	temp_datum['sentence'] = sa.augment_sentence(text_to_word_sequence(datum['sentence']), augmentation_rate)
	temp_datum['aspect'] = list(datum['aspect'])

	temp_data.append(temp_datum)

with open('data/absa/srilmAugmented05_aspect_train.json', 'w') as file:
