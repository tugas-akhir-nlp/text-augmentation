...
class BengioAugmenter():
	...
	def prob(self, sequences):
		...
		x = np.array([[x[0], x[0]]])
		y_pred = self.model.predict(x)[0][1]
...
if __name__=='__main__':
	ba = BengioAugmenter('model/cnw100_1.h5')
	augmentation_rate = 0.3

	print(ba.prob(text_to_word_sequence('Terios paling tembok')))
	
	# with open('data/absa/aspect_train.json') as file:
	# 	data = json.load(file)
	...
	# with open('data/absa/bengioAugmented_aspect_train.json', 'w') as file:
